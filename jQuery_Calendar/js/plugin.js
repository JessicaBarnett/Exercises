...
(function ( $ ) {
	...
	$.fn.calendar.setDefaultValues = function(date){
		...
		this.month = date.getMonth(); //1-12 for month 
	...
	$.fn.calendar.initializeCalendar = function(){
		return $('<table id="calendarTable"></table>').append('<tr>'+
			...
			'<th colspan="5">' + this.getMonthName(this.month) + " "  + this.year +'</th>'+
	...
	$.fn.calendar.printDate = function(){
		$("p#chosenDate").empty();
		$("p#chosenDate").append("You picked " + ($.fn.calendar.month+1) + "/" +  $(this).text() + "/" + $.fn.calendar.year);
	};
	...
	//these methods only change the first (preferably only) calendar in the page
	$.fn.calendar.prevMonth = function(date){
		$("#calendarTable").remove();
		$.fn.calendar.makeCalendar($("#calendar"), new Date($(this)[0].year, $(this)[0].month-1, 1));
	};
